...
두 열 다 하네스를 **켠** 상태다. 그러니 `fable-5` vs `sonnet-5` 격차는 "하네스 유무"가 아니라 **하네스를 켠 두 모델**의 차이 — 관심사는 *어디서* 갈리고 *근거가 무엇인가*다. 격차는 균일하지 않다:
